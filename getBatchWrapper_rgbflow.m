function [ fn ] = getBatchWrapper_rgbflow(opts, numThreads, trainopts)
...
% -------------------------------------------------------------------------
nFrames=imdb.images.nFrames(batch);
images = cell(1,numel(nFrames));
...
if isfield(trainopts, 'fetchGPU')
  opts.fetchGPU =  trainopts.fetchGPU;
end
...
if isfield(trainopts, 'subMedian')
    opts.subMedian = trainopts.subMedian;
end
    
im = cnn_get_im_flow_batch(images, opts, ...
